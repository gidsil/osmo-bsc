...
#if 1
		param = data;
		rc = gsm0808_assign_req(conn, *param >> 16, *param & 0xffff);
		osmo_fsm_inst_state_chg(fi, ST_WAIT_ASS_CMPL, 10, GSM0808_T10_VALUE);
#else
		if (chan_compat_with_mode(conn->lchan, chan_mode, full_rate)) {
			rc = gsm48_lchan_modify(conn->lchan, chan_mode);
			osmo_fsm_inst_state_chg(fi, ST_WAIT_MODE_MODIFY_ACK, 10, GSM0808_T10_VALUE);
		} else {
